...
public class MemberSendMessageHandler implements CIMMessageHandler{
	@Override
	public boolean process(Message message){
		String memberId = message.getSender();
		String messageContent = message.getContent();
		Member member = TeamMemberManager.getInstance().getMemberAnyway(memberId);
		RobotWrapper.getInstance().getRobotActionIntegration().receiveMessage(member, messageContent);
		return true;
	}
}
